...
from .encoders import EncoderWithAction, Encoder
...
class DoneModel(nn.Module):  # type: ignore
    """ done model.
     SVG
    """
    ...
    def compute_stats(
        ...
        xaction = torch.cat((x, action),1)
        h = self.network(xaction)
        return h

    def forward(
        self, x: torch.Tensor, action: torch.Tensor
    ) -> Tuple[torch.Tensor]:
        return self.predict_done(x, action)

    def predict_done(
        self, x: torch.Tensor, action: torch.Tensor
    ) -> torch.tensor:
        done = self.compute_stats(x, action)

        return done
    def perdict_done_sigmoid(
        self, x: torch.Tensor, action: torch.Tensor
    ...
    def compute_error(
        ...
        obs_t: torch.Tensor,
        act_t: torch.Tensor,
        done_tp1: torch.Tensor,
    ) -> torch.Tensor:
        batch_size,_= obs_t.shape
        done = self.compute_stats(obs_t, act_t)
        n_done = torch.sum(done_tp1)
        if n_done > 0.:
            pos_weight = ( batch_size- n_done) / n_done
        else:
            pos_weight = torch.tensor(1.)
        done_loss = F.binary_cross_entropy_with_logits(
            done, done_tp1, pos_weight=pos_weight,
            reduction='mean')
